...
def calculate_elo_rating(R_A, R_B, S_A, player_game_count=25):
    """
...
def generate_player_rankings():
    ...
    for match in matches:
            ...
            for player in ht_players:
                ...
                players[player].mmr = calculate_elo_rating(players[player].mmr, at_avg_mmr, score)
            for player in at_players:
                score = (away_score - home_score)/2 if away_score > home_score else 0
                players[player].mmr = calculate_elo_rating(players[player].mmr, ht_avg_mmr,score)
...
def generate_team_rankings_stratz(threshold=20):
    ...
    for match in matches:
            ...
            for pid in ht_players:
                print(f"{players[pid]['player_name']:<20} MMR: {players[pid]['mmr']}")
            print("=" * 40)
            print(f"Away Team: {away_team_name}")
            print("-" * 40)
            ...
            #home team players
            for pid in ht_players:
                if pid not in player_game_count:
                    player_game_count[pid] = 1
                else:
                    player_game_count[pid] += 1
                score = home_score if home_score > away_score else 0
                players[pid]['mmr'] = calculate_elo_rating(players[pid]['mmr'], at_avg_mmr, score, player_game_count[pid])
            ...
            for pid in at_players:
                ...
                players[pid]['mmr'] = calculate_elo_rating(players[pid]['mmr'], ht_avg_mmr,score, player_game_count[pid])
